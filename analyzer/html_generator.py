...
def _render_stock_cards(stocks: list, section: str = "section1") -> str:
    ...
    for i, stock in enumerate(stocks, 1):
        ...
        code_badge = ""
        if krx_code:
            code_badge = f'<span class="krx-code" data-code="{krx_code}" title="KRX 종목코드">{krx_code}</span>'
...
def _render_analyst_cards(stocks: list) -> str:
    ...
    for i, stock in enumerate(stocks, 1):
        ...
        if verified_price and verified_price.get("price"):
            p = verified_price
            change_val = p.get("change", "")
            sign = "▲" if change_val.startswith("+") else ("▼" if change_val.startswith("-") else "")
            change_display = change_val.lstrip("+-") if change_val else ""
            if sign and change_display:
                price_info_text = (
                    " (" + p["price"] + "원 " + sign + change_display + " " + p.get("change_pct", "") + ")"
                )
            elif p.get("price"):
        ...
        if krx_code:
            ...
            naver_url = "https://finance.naver.com/search/searchResult.naver?query=" + requests.utils.quote(name)
        chart_btn = (
            ' <a href="' + naver_url + '" target="_blank"'
        ...
        analyst_info_parts = []
        if broker:
            analyst_info_parts.append(f'<span class="analyst-broker">{broker}</span>')
        if analyst_name:
            analyst_info_parts.append(f'<span class="analyst-name">담당: {analyst_name}</span>')
        if target_price:
            analyst_info_parts.append(f'<span class="analyst-target">목표가: {target_price}원</span>')
        if opinion:
            opinion_class = "opinion-buy" if "BUY" in opinion.upper() or "매수" in opinion else (
                "opinion-sell" if "SELL" in opinion.upper() or "매도" in opinion else "opinion-hold"
            )
            analyst_info_parts.append(f'<span class="analyst-opinion {opinion_class}">{opinion}</span>')

        if analyst_info_parts:
            analyst_info = '<div class="analyst-info">' + " | ".join(analyst_info_parts) + '</div>'

        reasons_html = _render_reasons(reasons)

        code_badge = f'<span class="krx-code" data-code="{krx_code}">{krx_code}</span>' if krx_code else ""
...
<div class="container">
    ...
    <div class="header">
        <div class="header-top">
            ...
            <button class="refresh-btn" onclick="location.reload()" title="페이지 새로고침">&#x1F504; 새로고침</button>
        </div>
        <div class="date">최종 업데이트: {briefing_datetime}</div>
        <div class="desc">유튜브·미디어 채널 언급 종목 | 증권TV 전문가 추천 | 증권사 애널리스트 리포트를 통합 분석합니다.</div>
        <div class="nav-tabs">
    ...
    <div class="section" id="section1">
        <h2 class="section-title">&#x1F4FA; 섹션 1 <span class="section-sub">유튜브·미디어 채널 언급 종목 분석</span></h2>
        <div class="section-desc">
            지난 24시간 이내 방송사·개인유튜브·증권사 채널에서 언급된 KOSPI/KOSDAQ 상장 종목을 분석합니다.
    ...
    <div class="section" id="section2">
        <h2 class="section-title">&#x1F4E1; 섹션 2 <span class="section-sub">증권TV 전문가 출연 추천 종목</span></h2>
        <div class="section-notice">
            ※ 본 섹션은 전일({yesterday}) 방송 기준입니다.
            오전 방송이 오후에 업로드되는 특성을 고려하여 전일 방송 데이터를 기준으로 합니다.
        </div>
        {section2_html}
    </div>
    ...
    <div class="section" id="section3">
        <h2 class="section-title">&#x1F4C4; 섹션 3 <span class="section-sub">증권사 애널리스트 리포트 분석</span></h2>
        <div class="section-desc">
    ...
    <div class="section" id="section-strategy">
        ...
        <div class="section-desc">섹션 1(유튜브·미디어) + 섹션 2(증권TV) + 섹션 3(애널리스트 리포트) 통합 교차 분석</div>
...
.section-notice {
    ...
    background: #1a1a2e;
    border-radius: 8px;
    border-left: 2px solid #ffd43b60;
}
