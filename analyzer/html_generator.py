...
- BUG-H5    : hidden_picks signal 필터 — 긍정/매수/강력/상승/positive/buy 허용
...
- W-NEW-1   : _is_positive_signal 확장 (매수/강력/상승 계열 포함)
...
def _indicator_badge(label: str, value, pct, direction: str = "") -> str:
    ...
    BUG-9: pct == 0.0 도 유효한 숫자로 처리 (None 체크를 'is None'으로만 수행).
    """
    if value is None:
        return ""
...
def _build_section2_html(all_data: list) -> str:
    """
    경제방송TV / 경제방송 소스 항목에서 전문가 카드 렌더링.
    중복 제목 제거, 최대 20개.
    """
    if not all_data:
...
def _build_analyst_html(all_data: list) -> str:
    ...
    def _report_card(r: dict) -> str:
        ...
        summary_html = (
            f'<p class="analyst-summary">{summary}</p>'
            if summary else ""
        )
...
def _hidden_pick_source_badge(channel_type: str) -> str:
    """hidden_pick 소스 타입에 따른 인라인 스타일 배지 HTML 반환"""
...
def _safe_chart_key(prefix: str, name: str) -> str:
    """
    CR-NEW-1: 종목명의 특수문자·따옴표를 제거해 JS 객체 키로 안전한 문자열 반환.
...
def generate_html(
    ...
    Args:
        ...
        all_data       : 원본 수집 데이터 list (섹션2·3 렌더링용)
    """
    ...
    # Claude가 잘못 계산한 값을 channel_counts 기준으로 덮어씀
    for stock in stocks:
    ...
    # ── BUG-H5 + W-NEW-1: hidden_picks signal 필터 ───────────────────────────
    def _is_positive_signal(sig) -> bool:
        """
        긍정/매수/강력/상승/positive/buy 계열 시그널이면 True.
        None·빈 문자열은 False.
        """
        if not sig:
            return False
        sig_lower = str(sig).lower()
        positive_keywords = ("긍정", "매수", "강력", "상승", "positive", "buy")
        return any(kw in sig_lower for kw in positive_keywords)

    filtered_hidden = [
        h for h in hidden_picks
        if _is_positive_signal(h.get("signal"))
    ]
    ...
    for rank, stock in enumerate(filtered_stocks, start=1):
        ...
        if chart_b64:
            ...
            chart_btn_html = (
                ...
                f'onclick="showChart(\'{chart_key}\',\'{name}\')">'
                f'📈 차트 보기</button>'
            )
        elif naver_url:
            chart_btn_html = (
        ...
        # 사유 목록 — BUG-W-3: detail → reason → text 우선순위
        reasons_html = ""
        ...
        reasons_block = (
            f'<ul class="reasons-list">{reasons_html}</ul>'
...
"""
    ...
    for idx, hp in enumerate(filtered_hidden, start=1):
        ...
        hp_reasons_block = (
            f'<ul class="reasons-list">{hp_reasons_html}</ul>'
        ...
        if os.path.isdir(archive_dir) and gh_repo and "/" in gh_repo:
            ...
            if html_files:
                ...
                for fname in html_files:
                    ...
                    url = (
                        f"https://{owner}.github.io/{repo}/archive/{fname}"
                    )
                    archive_html += (
...
"""
    ...
    has_sec3     = bool(
        [d for d in all_data if d.get("source_type") == "애널리스트"]
    )
...
<head>
  ...
  <meta name="description"
        content="AI 기반 한국 주식 모닝브리핑 - {briefing_date}">
...
<div class="container">
  ...
  <header class="briefing-header">
    <h1>📈 AI 주식 모닝브리핑</h1>
    <p class="subtitle">
      {briefing_date} · {briefing_time} KST · 다중 채널 교차분석
    </p>
  </header>
...
<div class="modal-overlay" id="chartModal" onclick="closeChart(event)">
  <div class="modal-box">
    <button class="modal-close"
            onclick="document.getElementById('chartModal')
